...
class DashboardController extends Controller
{
    ...
    public function transpormes($empresa)
    {
        ...
        $fecha = Carbon::now();
        $datos = Transaccion::whereRaw('IDEmpresa = ? and YEAR(Fecha) = ? GROUP BY YEAR(Fecha), MONTH(Fecha) ', [$empresa, $fecha->year])
        ->get([DB::raw("SUM(Debe) as data, MONTHNAME(Fecha) as label")]);
